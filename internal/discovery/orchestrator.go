...
func (o *Orchestrator) stageExperiment(ctx context.Context, lane *Lane) error {
	o.emit(LaneProgress{LaneID: lane.ID, Angle: lane.Angle, State: StateExperiment,
		Message: fmt.Sprintf("Running %d experiment iterations…", o.opts.MaxRunsPerLane)})

	// Create a session for this lane
	sessionID := fmt.Sprintf("%s-%s", lane.Angle, time.Now().Format("20060102-1504"))
	sessionRoot := filepath.Join(o.root, ".research-loop", "sessions", sessionID)
	if err := os.MkdirAll(sessionRoot, 0755); err != nil {
		return fmt.Errorf("creating lane session dir: %w", err)
	}

	// Write hypothesis.md for the lane
	hypothesisContent := fmt.Sprintf(`# Hypothesis — %s

> Generated by Research Loop Discovery · %s
...
- [ ] Results reviewed
`, lane.Topic, time.Now().Format("2006-01-02 15:04"),
		lane.Topic, lane.Angle, lane.Claim, lane.Experiment,
		o.opts.RepoDir, o.opts.MaxRunsPerLane)

	if err := os.WriteFile(filepath.Join(sessionRoot, "hypothesis.md"), []byte(hypothesisContent), 0644); err != nil {
		return fmt.Errorf("writing hypothesis.md: %w", err)
	}

	// Write initial knowledge_graph.md
	kgContent := fmt.Sprintf(`# Knowledge Graph — %s

> Generated by Research Loop Discovery · %s

## Status
Session initialized for lane: %s
...
## Key Learnings
*None yet.*
`, lane.Angle, time.Now().Format("2006-01-02"), lane.Angle, lane.Claim)

	if err := os.WriteFile(filepath.Join(sessionRoot, "knowledge_graph.md"), []byte(kgContent), 0644); err != nil {
		return fmt.Errorf("writing knowledge_graph.md: %w", err)
	}

	// Write initial lab_notebook.md
	nbContent := fmt.Sprintf(`# Lab Notebook — %s
...
`, lane.Angle, time.Now().Format("2006-01-02"), time.Now().Format("2006-01-02 15:04"), lane.Topic, lane.Angle)

	if err := os.WriteFile(filepath.Join(sessionRoot, "lab_notebook.md"), []byte(nbContent), 0644); err != nil {
		return fmt.Errorf("writing lab_notebook.md: %w", err)
	}

	session := &persistence.Session{ID: sessionID, Root: sessionRoot}

	// Create a progress channel to receive updates from the runner
	progressCh := make(chan loop.Progress, 10)
	go func() {
		for p := range progressCh {
			o.emit(LaneProgress{
				...
				Message: fmt.Sprintf("Run #%d: %s", p.RunNumber, p.Message),
			})
		}
	}()

	// Create and run the loop runner
	runnerOpts := loop.Options{
		MaxRuns: o.opts.MaxRunsPerLane,
		RepoDir: o.opts.RepoDir,
		Resume:  false,
	}

	runner, err := loop.New(session, o.cfg, runnerOpts, progressCh)
	if err != nil {
	...
	if err := runner.Run(ctx); err != nil {
		o.emit(LaneProgress{LaneID: lane.ID, Angle: lane.Angle, State: StateExperiment,
			Message: fmt.Sprintf("Experiment loop ended: %v", err)})
	}

	close(progressCh)

	// Read the results from JSONL
	lane.Runs = o.collectLaneRuns(sessionRoot)
	if len(lane.Runs) > 0 {
		lane.BestMetric = lane.Runs[0].MetricVal
		lane.BestNode = lane.Runs[0].Node
		for _, r := range lane.Runs {
			if r.MetricVal < lane.BestMetric {
				lane.BestMetric = r.MetricVal
				lane.BestNode = r.Node
			}
		}
	}

	o.emit(LaneProgress{LaneID: lane.ID, Angle: lane.Angle, State: StateExperiment,
		Message: fmt.Sprintf("Completed %d runs, best=%.4f (%s)", len(lane.Runs), lane.BestMetric, lane.BestNode)})
...
func (o *Orchestrator) collectLaneRuns(sessionRoot string) []LaneRun {
	jsonlPath := filepath.Join(sessionRoot, "autoresearch.jsonl")
	...
	for _, line := range strings.Split(string(data), "\n") {
		...
		if v, ok := rec["run_number"].(float64); ok {
			run.RunNumber = int(v)
		}
		if v, ok := rec["node"].(string); ok {
			run.Node = v
		}
		if v, ok := rec["metric_value"].(float64); ok {
			run.MetricVal = v
		}
		if v, ok := rec["delta"].(float64); ok {
			run.Delta = v
		}
